...
class SinglePhaseGKS2D:
    """Isothermal 2D UGKS-style BGK solver for a periodic shear wave.

    Conserved variables are [rho, rho*u, rho*v].  The flux solver uses
    reconstructed macroscopic states for the equilibrium part and a discrete
    velocity distribution for the non-equilibrium/free-transport part.  This
    mirrors the Xu-Huang UGKS split: the time-integrated interface solution is
    a collisional Maxwellian contribution plus a reconstructed discrete
    distribution contribution.

    The implementation is written to keep Taichi kernels compact: each face
    computes the full three-component flux at once, and the benchmark-facing
    rho/u initialization and diagnostics are kept compatible with GKS.py.
    """

    def __init__(self, nx=256, ny=256, nvel=5):
        self.nx = nx
        self.ny = ny
        self.nvel = nvel
        self.dt = 0.01
        ...
        gh_x, gh_w = np.polynomial.hermite.hermgauss(self.nvel)
        self.velocity_nodes_np = (math.sqrt(2.0) * self.cs * gh_x).astype(np.float32)
        self.velocity_weights_np = (gh_w / math.sqrt(math.pi)).astype(np.float32)
        ...
        self.f = ti.field(ti.f32, shape=(self.nx, self.ny, self.nvel, self.nvel))
        self.f_next = ti.field(ti.f32, shape=(self.nx, self.ny, self.nvel, self.nvel))
        self.face_dist_x = ti.field(ti.f32, shape=(self.nx, self.ny, self.nvel, self.nvel))
        self.face_dist_y = ti.field(ti.f32, shape=(self.nx, self.ny, self.nvel, self.nvel))
        self.v_nodes = ti.field(ti.f32, shape=self.nvel)
        self.v_weights = ti.field(ti.f32, shape=self.nvel)
        self.face_x = ti.Vector.field(3, ti.f32, shape=(self.nx, self.ny))
        self.face_y = ti.Vector.field(3, ti.f32, shape=(self.nx, self.ny))
        self.v_nodes.from_numpy(self.velocity_nodes_np)
        self.v_weights.from_numpy(self.velocity_weights_np)
    ...
    @ti.func
    def discrete_maxwellian(self, rho, vel, vx, vy, wx, wy):
        cs2 = ti.static(self.cs2)
        exponent = (vx * vel[0] + vy * vel[1]) / cs2 - 0.5 * (vel[0] * vel[0] + vel[1] * vel[1]) / cs2
        return rho * wx * wy * ti.exp(exponent)

    @ti.func
    def slope_ratio(self, fm, f0, fp):
        slope = self.limited_scalar_slope(fm, f0, fp)
        return slope / ti.max(ti.abs(f0), 1.0e-12)
    ...
    @ti.func
    def equilibrium_distribution_local(self, rho_l, vel_l, grad_rho_l, grad_u_l, rho_r, vel_r, grad_rho_r, grad_u_r, vx, vy, wx, wy):
        mx_l = self.full_moments_1d(vel_l[0])
        my_l = self.full_moments_1d(vel_l[1])
        mx_r = self.full_moments_1d(vel_r[0])
        my_r = self.full_moments_1d(vel_r[1])
        ...
        g0 = self.discrete_maxwellian(rho0, vel0, vx, vy, wx, wy)
        inv_rho0 = 1.0 / ti.max(rho0, 1.0e-8)
        gx = g0 * (a0[0] + a0[1] * vx + a0[2] * vy) * inv_rho0
        gy = g0 * (b0[0] + b0[1] * vx + b0[2] * vy) * inv_rho0
        gt = g0 * (A0[0] + A0[1] * vx + A0[2] * vy) * inv_rho0
        return c_eq * g0 + c_eq_space * (vx * gx + vy * gy) + c_eq_time * gt
    ...
    def init(self):
        for i, j in self.rho:
            ...
            for a, b in ti.ndrange(self.nvel, self.nvel):
                vx = self.v_nodes[a]
                vy = self.v_nodes[b]
                wx = self.v_weights[a]
                wy = self.v_weights[b]
                self.f[i, j, a, b] = self.discrete_maxwellian(1.0, ti.Vector([ux, 0.0]), vx, vy, wx, wy)
                self.f_next[i, j, a, b] = self.f[i, j, a, b]
    ...
    def compute_face_fluxes_x(self):
        for i, j in self.rho:
            ...
            flux = self.equilibrium_flux_local(rho_l, vel_l, gr_l, gu_l, rho_r, vel_r, gr_r, gu_r)

            im = self.wrap_i(i - 1)
            ipp = self.wrap_i(ip + 1)
            jm = self.wrap_j(j - 1)
            jp = self.wrap_j(j + 1)
            tau = ti.static(self.tau)
            dt = ti.static(self.dt)
            E = ti.exp(-dt / tau)
            c_init = tau * (1.0 - E) / dt
            c_init_grad = -(tau * tau * (1.0 - E) - tau * dt * E) / dt
            ...
            for a, b in ti.ndrange(self.nvel, self.nvel):
                ...
                wx = self.v_weights[a]
                wy = self.v_weights[b]
                eq_bar = self.equilibrium_distribution_local(rho_l, vel_l, gr_l, gu_l, rho_r, vel_r, gr_r, gu_r, vx, vy, wx, wy)

                f0 = 0.0
                rx = 0.0
                ry = 0.0
                if vx >= 0.0:
                    sx = self.limited_scalar_slope(self.f[im, j, a, b], self.f[i, j, a, b], self.f[ip, j, a, b])
                    sy = self.limited_scalar_slope(self.f[i, jm, a, b], self.f[i, j, a, b], self.f[i, jp, a, b])
                    f0 = ti.max(self.f[i, j, a, b] + 0.5 * sx, 0.0)
                    rx = sx / ti.max(ti.abs(self.f[i, j, a, b]), 1.0e-12)
                    ry = sy / ti.max(ti.abs(self.f[i, j, a, b]), 1.0e-12)
                else:
                    sx = self.limited_scalar_slope(self.f[i, j, a, b], self.f[ip, j, a, b], self.f[ipp, j, a, b])
                    sy = self.limited_scalar_slope(self.f[ip, jm, a, b], self.f[ip, j, a, b], self.f[ip, jp, a, b])
                    f0 = ti.max(self.f[ip, j, a, b] - 0.5 * sx, 0.0)
                    rx = sx / ti.max(ti.abs(self.f[ip, j, a, b]), 1.0e-12)
                    ry = sy / ti.max(ti.abs(self.f[ip, j, a, b]), 1.0e-12)

                init_bar = c_init * f0 + c_init_grad * (vx * rx + vy * ry) * f0
                dist = ti.max(eq_bar + init_bar, 0.0)
                self.face_dist_x[i, j, a, b] = dist
                flux[0] += vx * init_bar
                flux[1] += vx * vx * init_bar
                flux[2] += vx * vy * init_bar

            self.face_x[i, j] = flux
    ...
    def compute_face_fluxes_y(self):
        for i, j in self.rho:
            ...
            flux = ti.Vector([flux_l[0], flux_l[2], flux_l[1]])

            jm = self.wrap_j(j - 1)
            jpp = self.wrap_j(jp + 1)
            im = self.wrap_i(i - 1)
            ip = self.wrap_i(i + 1)
            tau = ti.static(self.tau)
            dt = ti.static(self.dt)
            ...
            for a, b in ti.ndrange(self.nvel, self.nvel):
                ...
                wx = self.v_weights[a]
                wy = self.v_weights[b]
                eq_bar = self.equilibrium_distribution_local(rho_l, vel_l, gr_l, gu_l, rho_r, vel_r, gr_r, gu_r, vy_w, vx_w, wy, wx)
                ...
                if vy_w >= 0.0:
                    sx = self.limited_scalar_slope(self.f[im, j, a, b], self.f[i, j, a, b], self.f[ip, j, a, b])
                    sy = self.limited_scalar_slope(self.f[i, jm, a, b], self.f[i, j, a, b], self.f[i, jp, a, b])
                    f0 = ti.max(self.f[i, j, a, b] + 0.5 * sy, 0.0)
                    rx = sx / ti.max(ti.abs(self.f[i, j, a, b]), 1.0e-12)
                    ry = sy / ti.max(ti.abs(self.f[i, j, a, b]), 1.0e-12)
                else:
                    sx = self.limited_scalar_slope(self.f[im, jp, a, b], self.f[i, jp, a, b], self.f[ip, jp, a, b])
                    sy = self.limited_scalar_slope(self.f[i, j, a, b], self.f[i, jp, a, b], self.f[i, jpp, a, b])
                    f0 = ti.max(self.f[i, jp, a, b] - 0.5 * sy, 0.0)
                    rx = sx / ti.max(ti.abs(self.f[i, jp, a, b]), 1.0e-12)
                    ry = sy / ti.max(ti.abs(self.f[i, jp, a, b]), 1.0e-12)

                init_bar = c_init * f0 + c_init_grad * (vx_w * rx + vy_w * ry) * f0
                dist = ti.max(eq_bar + init_bar, 0.0)
                self.face_dist_y[i, j, a, b] = dist
                flux[0] += vy_w * init_bar
                flux[1] += vy_w * vx_w * init_bar
                flux[2] += vy_w * vy_w * init_bar

            self.face_y[i, j] = flux
    ...
    def update_distribution(self):
        for i, j, a, b in self.f:
            ...
            transport = vx * (self.face_dist_x[i, j, a, b] - self.face_dist_x[im, j, a, b])
            transport += vy * (self.face_dist_y[i, j, a, b] - self.face_dist_y[i, jm, a, b])

            g_old = self.discrete_maxwellian(self.rho[i, j], self.u[i, j], vx, vy, wx, wy)
            g_new = self.discrete_maxwellian(self.rho_next[i, j], self.u_next[i, j], vx, vy, wx, wy)
            tau = ti.static(self.tau)
            dt = ti.static(self.dt)
            f_old = self.f[i, j, a, b]
            rhs = f_old - dt * transport + 0.5 * dt * (g_old - f_old) / tau + 0.5 * dt * g_new / tau
            self.f_next[i, j, a, b] = ti.max(rhs / (1.0 + 0.5 * dt / tau), 0.0)
    ...
    def swap(self):
        ...
        for i, j, a, b in self.f:
            self.f[i, j, a, b] = self.f_next[i, j, a, b]
...
def main():
    ...
    sim.init()
    gui = ti.GUI("Unified BGK-UGKS", res=(sim.nx, sim.ny), fast_gui=False)
    s = 0
    sim_time = 0.0
    while gui.running:
        # print(s, sim_time)
