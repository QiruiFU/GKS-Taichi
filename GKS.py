...
class SinglePhaseGKS2D:
    """Full second-order isothermal BGK-GKS for a periodic 2D shear wave.

    Conserved variables are [rho, rho*u, rho*v].  The flux solver uses
    reconstructed left/right states, half-space compatibility for the
    interface Maxwellian, Chapman-Enskog initial distributions, and the
    time-averaged analytic BGK integral solution.

    The implementation is written to keep Taichi kernels compact: each face
    computes the full three-component flux at once, and Maxwellian moments are
    precomputed once per state instead of expanded repeatedly.
    """
    ...
    def compute_face_fluxes_x(self):
        for i, j in self.rho:
            ...
            self.face_x[i, j] = self.gks_flux_local(rho_l, vel_l, gr_l, gu_l, rho_r, vel_r, gr_r, gu_r)
    ...
    def compute_face_fluxes_y(self):
        for i, j in self.rho:
            ...
            flux_l = self.gks_flux_local(rho_l, vel_l, gr_l, gu_l, rho_r, vel_r, gr_r, gu_r)
...
def main():
    ...
    sim.init()
    gui = ti.GUI("Full BGK-GKS", res=(sim.nx, sim.ny), fast_gui=False)
    s = 0
    sim_time = 0.0
    while gui.running:
        # print(s, sim_time)
